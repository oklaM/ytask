...
const TaskFormNew: React.FC = () => {
  ...
  const handleParseComplete = (result: any) => {
    ...
    // 自动填充表单
    const { time, task } = result;
    
    // 设置任务基本信息
    form.setFieldsValue({
      name: `智能解析任务 - ${new Date().toLocaleTimeString()}`,
      description: task.description || result.description || '智能解析任务',
      category: '智能任务'
    });
    ...
    if (task.type === 'http') {
      form.setFieldsValue({
        httpUrl: task.config?.url || '',
        httpMethod: task.config?.method || 'GET',
        httpHeaders: task.config?.headers ? JSON.stringify(task.config.headers, null, 2) : '',
        httpBody: task.config?.body ? JSON.stringify(task.config.body, null, 2) : ''
      });
    } else if (task.type === 'command') {
      form.setFieldsValue({
  ...
  return (
    <div>
      ...
      {/* 智能任务解析 */}
      <SmartTaskParser onParseComplete={handleParseComplete} />
